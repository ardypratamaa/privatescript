...
function addNewPlayerById(id, auth, stars) {
  playersbintang[id] = {
      id: id,
      auth: auth,
      stars: stars 
      ...
      if (xhr.readyState === XMLHttpRequest.DONE) {
          if (xhr.status === 200) {
              ...
              if (response.success) {
                  ...
                  callback(playerData.auth, playerData.stars);
              } else {
                  callback(null); 
              }
          } else {
...
room.onPlayerJoin = function (player) {
  ...
  checkPlayerInDatabase(auth, function(databaseAuth, databaseStars) {
      if (databaseAuth) {
          addNewPlayerById(id, databaseAuth, databaseStars);
          playersbintang[id].id = id; 
          console.log("Player " + player.name + " rejoined. Stars: " + databaseStars);
          setTimeout(() => {
            room.sendAnnouncement("Welcome " + player.name + ", You have " + databaseStars + " stars.", player.id, 0x5CF49C, "small");
          }, 3500);
      } else {
          addNewPlayerById(id, auth, 10);
          console.log("Player " + player.name + " bergabung untuk pertama kali. Stars: 10");
          setTimeout(() => {
